...
def client_retraining(client_number, dimension, level, Nof_feature,
    ...
    * too large size may cause the quality become bad )
    ...
    # Retrain the AM
    _, Times = MNIST.retrain(test_x=train_label[batch_index], test_y=val_label, train_x=train_data[batch_index], train_y=train_label[batch_index], num_epoch=1, train_acc_demand=0.7, batch_size=batch_size, save_path=os.path.join(os.path.join(os.path.dirname(
