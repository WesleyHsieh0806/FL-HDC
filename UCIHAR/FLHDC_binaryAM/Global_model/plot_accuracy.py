...
y = [0.01*i for i in range(78, 95)]
plt.plot(x, result[:len(result)], color='r', marker='o', linewidth=3,
         label="FLHDC-Binary")
plt.plot(x, accuracy[:len(result)], color='yellowgreen', linewidth=3,
...
with open(os.path.join(Dir_cen, 'binary_retrain_60000+10000/Accuracy10000.csv'), 'r') as f:
    for line in f:
        # append the average of results for each parameter setup into the accuracy list

        centralized.append(np.array(line.strip().strip(
            ',').split(','), dtype=float))
# Get the accuracy of centralized HDC on UCIHAR
...
plt.plot(range(1, len(centralized)+1), centralized, color='r', marker='o', linestyle='--',
