...
'''
Author: Wesley Hsieh
This file is for experiments of Centralized HDC on EUCIHAR Dataset
Experiments includes:
1.Centralized HDC on Total IID UCIHAR Dataset
2.Cetralized HDC+Retrain on Total IID UCIHAR Dataset
...
def main():
    train_data_path = os.path.join(os.path.dirname(
        __file__), "../../UCIHAR/data_csv/train_data.csv")
    train_label_path = os.path.join(os.path.dirname(
        __file__), "../../UCIHAR/data_csv/train_label.csv")
    test_data_path = os.path.join(os.path.dirname(
        __file__),  "../../UCIHAR/data_csv/test_data.csv")
    test_label_path = os.path.join(os.path.dirname(
        __file__), "../../UCIHAR/data_csv/test_label.csv")
    # Read training data
    ...
    for i in range(average_time):
        for dimension in Dimension:
            ...
            UCIHAR = HDC.HDC(dimension, n_of_class, n_of_feature,
            ...
            start = time.time()
            y_pred = UCIHAR.test(test_x[:])
            test_time = time.time()-start
            acc = UCIHAR.accuracy(y_true=test_y[:], y_pred=y_pred[:])
            ...
            # UCIHAR.load_model('HDC_model.pickle')
            # Retrain
            print("{:=^40}".format("Start Retraining"))
            _, acc_history, time_history = UCIHAR.retrain(
