...
def main():
    ...
    if not os.path.isdir('../Result/train_8000+4000'):
        os.makedirs('../Result/train_8000+4000')
    """
        HDC Training Part
    """
    '''# Parameter Setup'''
    ...
    for dimension in Dimension:
        ...
        Acc.to_csv('../Result/train_8000+4000/Accuracy' +
                   str(dimension)+'.csv')
        TTime = pd.DataFrame([Time[str(dimension)+' time']],
                             index=[0], columns=[dimension])
        TTime.to_csv('../Result/train_8000+4000/Training_time' +
                     str(dimension)+'.csv')

    # Save the accuracy and training time into csv file
    result = pd.DataFrame(result, index=[0])
    result.to_csv('../Result/train_8000+4000/Accuracy.csv')
    Time = pd.DataFrame(Time, index=[0])
    Time.to_csv('../Result/train_8000+4000/Training_time.csv')
