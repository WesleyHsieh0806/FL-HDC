...
def main():
    ...
    * Load "Setup.pickle" to acquire the parameter setup 
    ...
    # case2: retrain
        ...
        * as this formula: Ck = Ck + sigma(learning rate * Times_add * Rkj_add + learning_rate * Time_sub * Rkj_sub)  (Rkj:retrain_vectors of class-k from client j) 
        '''
        ...
        for Class in range(nof_class):
            Total_times_add[Class] = 0
            Total_times_sub[Class] = 0
        ...
        for client in range(1, nof_clients+1):
            with open(os.path.join(os.path.join(os.path.join(os.path.dirname(__file__), '..'), 'client'+str(client)), 'Upload.pickle'), 'rb') as f:
                ...
                client_dict = pickle.load(f)
            for label in range(nof_class):
                # Load Time_add and Times_sub for the label
                Times_add = client_dict['Times_add'+str(label)]
                Times_sub = client_dict['Times_sub'+str(label)]
                # Update the integer AM
                # if label == 1:
                #     Prototype_vector['integer'][label] += int(learning_rate[label] * Times_add) * \
                #         client_dict['Retrain_vector_add'][label] + int(np.maximum((learning_rate[label] - 2), 1) * Times_sub) * \
                #         client_dict['Retrain_vector_sub'][label]

                Prototype_vector['integer'][label] += int(learning_rate[label] * Times_add) * \
                    client_dict['Retrain_vector_add'][label] + int(learning_rate[label] * Times_sub) * \
                    client_dict['Retrain_vector_sub'][label]
                # add the size to total modification times
                Total_times_add[label] += Times_add
                Total_times_sub[label] += Times_sub
                # test the quality of class1 and class2 retrain_vector
                if label == 1:
                    print("# of different bits between class1 hv and R_add:{}".format(np.count_nonzero(
                        client_dict['Retrain_vector_add'][label] != Prototype_vector['binary'][label])))
                    print("# of different bits between class1 hv and R_sub:{}".format(np.count_nonzero(
                        client_dict['Retrain_vector_sub'][label] != Prototype_vector['binary'][label])))
                if label == 2:
                    print("# of different bits between class2 hv and R_add:{}".format(np.count_nonzero(
                        client_dict['Retrain_vector_add'][label] != Prototype_vector['binary'][label])))
                    print("# of different bits between class2 hv and R_sub:{}".format(np.count_nonzero(
                        client_dict['Retrain_vector_sub'][label] != Prototype_vector['binary'][label])))
        # Print out the total modification times
        for Class in range(nof_class):
            print("Class:{} Total times of add:{} sub:{}".format(
                Class, Total_times_add[Class], Total_times_sub[Class]))
