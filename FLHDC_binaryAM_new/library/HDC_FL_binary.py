...
class HDC:
    ...
    def retrain_initial_model(self, test_x, test_y, train_x, train_y, num_epoch=5, batch_size=400, save_path='HDC_model.pickle'):
        ''' 
        ...
        This retrain is only used for global model initialization
    ...
    def Init_Retrain_vector(self):
        ''' 
        * Initialize retrain_vectors and cancel the IntegerAM from Global Model
        * In the setting of FL-Binary, we only obtain BinaryAM from Global Model
        '''
        # initialize Retrain vector add and sub
        self.retrain_vector_add = {}
        self.retrain_vector_sub = {}
        # Cancel the Integer AM from GLobal Model to prevent misuse
        ...
        for i in range(0, self.nof_class):
            ...
            self.retrain_vector_add[i] = np.zeros(
                self.Prototype_vector['binary'][i].shape).astype(int)
            self.retrain_vector_sub[i] = np.zeros(
                self.Prototype_vector['binary'][i].shape).astype(int)
    ...
    def retrain(self, test_x, test_y, train_x, train_y, num_epoch=5, train_acc_demand=0.85, batch_size=400, save_path='HDC_model.pickle'):
        ...
        acc = 0.
        total_batch = ceil(len(train_x)/batch_size)
        # Initialize Retrain_vectors
        self.Init_Retrain_vector()
        # Initialize times (times[class] indicates the number of times the retrain_vector[class] has been modified)
        Times_add = {}
        Times_sub = {}
        for i in range(self.nof_class):
            Times_add[i] = 0
            Times_sub[i] = 0
        if self.PCA_projection:
            # in retrain phase, we have to project the training data first if PCA was used in training
        ...
        for retrain_epoch in range(1, num_epoch+1):
            ...
            for batch in range(total_batch):
                ...
                for data in range(batch_size):
                    ...
                    if data_index < len(train_x):
                        ...
                        if predicted_class != train_y[data_index][0]:
                            # In this case : predicted_class = mismatched class
                            # the real class is train_y[data]
                            # we collect these encoded hv to be further upload to global_model to do retrain
                            self.retrain_vector_sub[predicted_class] -= query_vector
                            self.retrain_vector_add[train_y[data_index]
                                                    [0]] += query_vector
                            Times_sub[predicted_class] += 1
                            Times_add[train_y[data_index]
                                      [0]] += 1
                    else:
                        # All data have been encoded
                        break
                ...
                for CLASS in range(0, self.nof_class):
                    ...
                    # As a result, we have to binarize them (>0 --> 1   <0 --> -1)
                    # Special case: if an element is 0, then randomly change it into 1 or -1
                    self.retrain_vector_add[CLASS][self.retrain_vector_add[CLASS]
                                                   >= 0] = 1
                    self.retrain_vector_add[CLASS][self.retrain_vector_add[CLASS]
                                                   < 0] = -1
                    self.retrain_vector_sub[CLASS][self.retrain_vector_sub[CLASS]
                                                   >= 0] = 1
                    self.retrain_vector_sub[CLASS][self.retrain_vector_sub[CLASS]
                                                   < 0] = -1
                ''' Print the training_accuracy for each batch'''
                ...
                print("Last accuracy:{:.4f}".format(
                    last_acc))
                # Compare Times_add vs Times_sub
                print("Class1 Times_add:{} Times_sub:{}".format(
                    Times_add[1], Times_sub[1]))
                print("Class2 Times_add:{} Times_sub:{}".format(
                    Times_add[2], Times_sub[2]))
        print("\nRetrain Complete! Best accuracy:{:.4f}".format(best_acc))
        return best_acc, Times_add, Times_sub
    ...
    def hamming_distance(self, Query_vector, Prototpye_vector):
        '''
        Return dimension-hamming distance of Query_vector and Prototype)vector
        What we want is "similarity", and the minimum hamming distance would have the maximum similarity.
        As a result, we return dimension-hamming distance instead of hamming distance
        '''
        # All elements in Query vector and Prototype vector are 1 or -1
        # By using array == 1, we transform array into binary(with 1 becoming True and -1 becoming False)
        return self.nof_dimension - np.sum(np.logical_xor((Query_vector == 1), (Prototpye_vector == 1)))
