...
def client_retraining(client_number, dimension, level, Nof_feature,
    ...
    * too large size may cause the quality become bad )
    ...
    batch_size = np.minimum(
        len(train_data), (len(train_data))//2)
    batch_index = np.random.choice(
        range(len(train_data)), batch_size, replace=False)
    # Retrain the AM
    _, Times_add, Times_sub = ISOLET.retrain(test_x=train_data[batch_index], test_y=train_label[batch_index], train_x=train_data[batch_index], train_y=train_label[batch_index], num_epoch=1, train_acc_demand=0.7, batch_size=batch_size, save_path=os.path.join(os.path.join(os.path.dirname(
    ...
    Upload_to_Server['Retrain_vector_add'] = ISOLET.retrain_vector_add
    Upload_to_Server['Retrain_vector_sub'] = ISOLET.retrain_vector_sub
