...
def LR_Decider(flipped_bit, learning_rate, nof_class):
    ''' Adjust the learning rate by # of flipped bit last time
    * the learning rate will lie in these given values [1, 2, 3] 
...
def main():
    ...
    * Load "Setup.pickle" to acquire the parameter setup 
        ...
        * as this formula: Ck = Ck + sigma(learning rate * Times_add * Rkj_add + learning_rate * Time_sub * Rkj_sub)  (Rkj:retrain_vectors of class-k from client j) 
        ...
        if os.path.isfile(os.path.join(file_dir, 'flipped_bit.pickle')):
            ...
            learning_rate = LR_Decider(
                flipped_bit, learning_rate, nof_class=nof_class)
        else:
    ...
    ISOLET = HDC.HDC(dimension, nof_class, Nof_feature, level)
    ISOLET.load_model(os.path.join(file_dir, 'global_model_dict.pickle'))
    ''' load test dataset'''
    test_data, test_label = load_data()
    y_pred = ISOLET.test(test_x=test_data)
